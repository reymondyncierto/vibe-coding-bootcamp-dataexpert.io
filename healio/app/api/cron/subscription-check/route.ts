...
import { runSubscriptionGracePeriodCheck } from "@/services/subscriptionService";
...
async function run(request: Request) {
  ...
  const graceDaysRaw = Number(url.searchParams.get("graceDays") || "3");
  const graceDays = Number.isFinite(graceDaysRaw) ? Math.max(0, Math.floor(graceDaysRaw)) : 3;

  return successResponse(runSubscriptionGracePeriodCheck({ now, graceDays }));
}
